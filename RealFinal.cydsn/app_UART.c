...
int rxindex = 0;                        // UART receive buffer index
    
double number;                      // floating-point number
    
uint8 ch  = 0u;                     // variable to hold received character
...
*
* Summary:
*  This function sends a string. We're all very proud.
*  
*
* Parameters:
...
void HandleUartTxTraffic() {
    ...
    uartTxDataLength = strlen(sentString);
    ...
    if(0 != uartTxDataLength)
    {
        ...
        if(0 != uartTxDataLength)
        {
            ...
            {
                uartTxData[index] = sentString[index];
            }
